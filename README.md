# Business Statistics — Interactive Applets

Built as single-file HTML — no installation, no login, works in any modern browser.

## Open the applets (one click)

| Applet | Use it for | Session |
|---|---|---|
| [Binomial Explorer](https://parasuram-bala.github.io/Applets-Business-Statistics/binomial_explorer.html) | Yes/no counts over n trials — QC batches, conversion tests | 3 |
| [Poisson Explorer](https://parasuram-bala.github.io/Applets-Business-Statistics/poisson_explorer.html) | Counts per time window — call-centre arrivals, web orders | 3 |
| [Normal Explorer](https://parasuram-bala.github.io/Applets-Business-Statistics/normal_explorer.html) | Continuous outcomes — delivery times, fill weights | 3 |
| [Sampling Distribution Explorer](https://parasuram-bala.github.io/Applets-Business-Statistics/sampling_explorer.html) | Watch the sampling distribution of x̄ build, one sample at a time — the CLT in action | 4 |

## How to use

1. Click a link above — the applet opens in your browser.
2. Pick a business scenario (chips at the top right), set the parameters, and read the
   probability off the chart.
3. In the Sampling Distribution Explorer: click **Draw 1 sample** a few times first,
   then **Draw 1,000** — the histogram of sample means *is* the sampling distribution.

**Offline use:** download any `.html` file (Code → Download ZIP) and double-click it —
everything runs locally, no internet needed.

---

*Prof. Parasuram Balasubramanian · BITSoM 
